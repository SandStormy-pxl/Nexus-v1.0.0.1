...
class Story(models.Model):
    ...
    def __str__(self):
        return f'Story #{self.id} — @{self.autor.username}'


class StoryVisualizacao(models.Model):
    usuario = models.ForeignKey(User, on_delete=models.CASCADE, related_name='stories_vistas')
    story = models.ForeignKey(Story, on_delete=models.CASCADE, related_name='visualizacoes')
    criado_em = models.DateTimeField(auto_now_add=True)

    class Meta:
        unique_together = ('usuario', 'story')

    def __str__(self):
        return f'@{self.usuario.username} viu Story #{self.story_id}'
